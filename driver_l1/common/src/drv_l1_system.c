...
void system_bus_arbiter_init(void)			//系统仲裁初始化
{
	/*
	0 ppu display
	1 sensor
...
void sys_power_handler(void)
{
	...
	while (1)
	{
		...
		if (ir_cnt > 20)
		{ //power on by a short press of 200ms
			break;
		}
...
INT32S sys_msdc_clk_active(void)
{
	...
	// sensor
#if (								sensor_format == 1)		// usually symbol '1' is parallel interface(并行接口), '2' is MIPI interface
