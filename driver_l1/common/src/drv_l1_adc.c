...
static void adc_auto_isr()
{
	...
	if ((R_MIC_ASADC_CTRL & MIC_ASIF) && (R_MIC_ASADC_CTRL & MIC_ASIEN))
	{ //add by duxiatang,2009-12-09
...
enum I2SRX_ISCR_REG
{
	...
	I2SRX_ClearFIFO = 0x40000,							/* High active FIFO clear bit, user need set 0 after clear(always 
	on) */
	I2SRX_MERGE = 0x80000,								/* 32 bit merge select */
	I2SRX_R_LSB = 0x100000, 							/* Right channel data in LSB of RX_Data or not, only valid when 
	word length is 16-bit wide */
	I2SRX_Mono = 0x200000								/* Right channel data in LSB of RX_Data or not, only valid when 
	word length is 16-bit wide */
};
