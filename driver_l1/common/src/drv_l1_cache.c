...
static void cache_drain_write_buffer(void)
{
	...
	if ((R_CACHE_CTRL & (C_CACHE_CTRL_EN | C_CACHE_CTRL_WRITE_BUFFER_EN)) ==
		 (C_CACHE_CTRL_EN | C_CACHE_CTRL_WRITE_BUFFER_EN))
	{
...
INT32S cache_lockdown(INT32U addr, INT32U size)
{
	...
	while (size)
	{
		...
		{
			val_lock_tag		=
				 * ((INT32U *) (vlt_reg + (bank << C_CACHE_VLT_BANK_SHIFT) + (addr & C_CACHE_INDEX_MASK)));
		...
		if (candidate < C_CACHE_BANK_NUM)
		{ // Found a bank for locking down the line into cache
...
static INT32S cache_drain_line(INT32U bank, INT32U addr)
{
	...
	if (bank >= C_CACHE_BANK_NUM)
	{ // Bank = 0, 1, ... C_CACHE_BANK_NUM-1
		return - 1;
	}

	index				= addr & C_CACHE_INV_LINE_INDEX_MASK;
	R_CACHE_INVALID_LINE =
		 C_CACHE_INV_LINE_EN | (bank << C_CACHE_INV_LINE_BANK_SHIFT) | index | C_CACHE_INV_LINE_DRAIN;
...
static INT32S cache_invalid_line(INT32U bank, INT32U addr)
{
	...
	index				= addr & C_CACHE_INV_LINE_INDEX_MASK;
	R_CACHE_INVALID_LINE =
		 C_CACHE_INV_LINE_EN | (bank << C_CACHE_INV_LINE_BANK_SHIFT) | index | C_CACHE_INV_LINE_INVALID;
