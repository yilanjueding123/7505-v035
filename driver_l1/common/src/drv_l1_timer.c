...
static INT32U timebase_trap[3] =
{
	0
};


static INT32U timebase_trap_counter[3] =
{
	0
};


static INT32U time_base_isr_avoid[3] =
{
	TIME_BASE_FALSE
};


static INT32U time_base_times_isr_avoid[3] =
{
	TIME_BASE_FALSE
};
	...
	INT32U EXTBSEL, INT32U EXTASEL, INT32U TMXEN, INT32U TMXIE, INT32U TMXIF)
{
	...
	reg_value			|=
		 (SRCASEL << 0 | SRCBSEL << 4 | EXTBSEL << 8 | EXTASEL << 10 | TMXEN << 13 | TMXIE << 14 | TMXIF << 15);
...
INT32S time_base_control(INT32U TIME_BASE_ID, TMBXS_ENUM TMBXS, INT32U TMBXEN, INT32U TMBXIE, INT32U TMBXIF)
{
	...
	if (TIME_BASE_ID < TIME_BASE_ID_MAX) /* Timer id  =  {Timer A, Timer B , Timer C, Timer D} which included in 
		Timer1 reg.*/
	{
...
void local_time_base_isr0(void)
{
	...
	if (((R_TIMEBASEA_CTRL & TIMER_INT_FLAG_MASK) == TIMER_INT_FLAG_MASK) &&
		 (time_base_isr_avoid[TIME_BASE_A] != TIME_BASE_TRUE))
	{
	...
	if (((R_TIMEBASEB_CTRL & TIMER_INT_FLAG_MASK) == TIMER_INT_FLAG_MASK) &&
		 (time_base_isr_avoid[TIME_BASE_B] != TIME_BASE_TRUE))
	{
	...
	if (((R_TIMEBASEC_CTRL & TIMER_INT_FLAG_MASK) == TIMER_INT_FLAG_MASK) &&
		 (time_base_isr_avoid[TIME_BASE_C] != TIME_BASE_TRUE))
	{
