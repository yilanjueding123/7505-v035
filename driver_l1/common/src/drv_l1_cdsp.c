...
__align(16)
INT32U md_work_memory_addr[3616 / 4];
...
void hwCsdp_EnableDynGClk(INT8U enable)
{
	if (enable == 1)
...
void hwCdsp_RedoTriger(INT8U docdsp)
{
	if (docdsp)
		R_CDSP_DO |= 0x01;
	else 
...
void hwCdsp_SetGatingValid(INT8U gating_gamma_vld, 
	INT8U gating_badpixob_vld)
{
	R_CDSP_DATA_FORMAT	&= ~(0x03 << 8);
	R_CDSP_DATA_FORMAT	|= ((INT32U) gating_gamma_vld & 0x1) << 8 | ((INT32U) gating_badpixob_vld & 0x1) << 9;
}


void hwCdsp_SetDarkSub(INT8U raw_sub_en, 
	INT8U sony_jpg_en)
{
	R_CDSP_DATA_FORMAT	&= ~0x0880;
	R_CDSP_DATA_FORMAT	|= (raw_sub_en & 0x1) << 7 | ((INT32U) sony_jpg_en & 0x1) << 11;
}


/*U2 Added*/
void hwCdsp_SetOutYUVFmt(INT8U yuv_format //out sequence, 0:YUYV, 1:UYVY, 2:YVYU, 3:VYUY
)
{
...
void hwCdsp_SetRawDataFormat(INT8U format)
{
...
void hwCdsp_SetYuvRange(INT8U signed_yuv_en)
{
...
void hwCdsp_SetSRAM(INT8U overflowen, 
	INT16U sramthd)
{
	if (sramthd > 0x1FF)
		sramthd = 0x100;
...
interrupt
********************/
void hwCdsp_SetInt(INT8U inten, 
	INT8U bit)
{
	if (inten)
	{
...
other
********************/
void hwCdsp_SetLineInterval(INT16U line_interval)
{
	R_CDSP_LINE_INTERVAL = line_interval;
}


void hwCdsp_SetExtLine(INT8U extinen, 
	INT16U linesize, 
	INT16U lineblank)
{
	//DBG_PRINT("<ExtLine>\r\n");
...
path set
********************/
void hwCdsp_DataSource(INT8U image_source)
{
...
* @return	none
*/
void hwCdsp_SetRawPath(INT8U raw_mode, //rgb path
INT8U cap_mode, //0: package 10 to 16, 1:package 8 to 16  
INT8U yuv_mode //0:8y4u4v, 1: yuyv
)
{
	R_CDSP_DATA_FORMAT	&= ~0x3F;					/*0x240*/
	R_CDSP_DATA_FORMAT	|= (raw_mode & 0x07) | (cap_mode & 0x1) << 4 | (yuv_mode & 0x1) << 5;
}


void hwCdsp_SetYuvMuxPath(INT8U redoedge)
{
	if (redoedge)
...
void hwCdsp_SetLensCmpPath(INT8U yuvlens)
{
...
void hwCdsp_SetExtLinePath(INT8U path)
{
	if (path)
...
void hwCdsp_SetLineCtrl(INT8U ybufen //0: raw data, 1: YUV data	
)
{
	if (ybufen)
...
cdsp dma buffer set
*********************/
void hwCdsp_SetYuvBuffA(INT16U width, 
	INT16U height, 
	INT32U buffer_addr)
{
...
void hwCdsp_SetYuvBuffB(INT16U width, 
	INT16U height, 
	INT32U buffer_addr)
{
...
INT32U width, 
	...
	INT32U buffer_addr)
{
	R_CDSP_DMA_RAWBUF_HVSIZE = (((INT32U) width * 8 / 16) & 0xFFF) | ((((INT32U) height * 8 / 16) & 0xFFF) << 12);
...
INT32U width, 
	...
	INT32U buffer_addr)
{
	R_CDSP_DMA_RAWBUF_HVSIZE =
		 (((INT32U) (width * 10 / 16)) & 0xFFF) | (((INT32U) ((height * 10 / 16)) & 0xFFF) << 12);
	R_CDSP_DMA_RAWBUF_HOFFSET = hoffset;
	R_CDSP_DMA_RAWBUF_SADDR = buffer_addr;
}


void hwCdsp_SetDmaBuff(INT8U buffer_mode)
{
...
void hwCdsp_SetReadBackSize(INT16U hoffset, 
	INT16U voffset, 
	INT16U hsize, 
	INT16U vsize)
{
...
void hwCdsp_SetReadBackSize10(INT16U hoffset, 
	INT16U voffset, 
	INT16U hsize, 
	INT16U vsize)
{
...
clamp set
***************/
void hwCdsp_SetRBClamp(INT8U rbclampen, 
	INT16U rbclamp)
{
...
void hwCdsp_EnableClamp(INT8U clamphsizeen, 
	INT16U Clamphsize)
{
	/* Enable clamping */
...
void hwCdsp_SetUVScale(INT8U uvDiven, 
	INT8U yfrcuvdiv1_8, 
	INT8U yfrcuvdiv2_8, 
	INT8U yfrcuvdiv3_8, 
	INT8U yfrcuvdiv4_8, 
	...
	INT8U yfrcuvdiv6_8)
{
	...
	{
		R_CDSP_UVSCALE_COND1 = ((INT32U) yfrcuvdiv3_8 << 16) | ((INT32U) yfrcuvdiv2_8 << 8) | yfrcuvdiv1_8;
		R_CDSP_UVSCALE_COND0 = ((INT32U) yfrcuvdiv6_8 << 16) | ((INT32U) yfrcuvdiv5_8 << 8) | yfrcuvdiv4_8;
		R_CDSP_YUV_CTRL 	|= uvDiven & 0x01;
	}
}


void hwCdsp_QcntThr(INT16U Qthr, 
	INT16U PreRBclamp)
{
...
bad pixel set
*********************/
void hwCdsp_EnableBadPixel(INT8U badpixen, 
	INT8U badpixmiren, 
	INT8U badpixmirlen)
{
...
void hwCdsp_SetBadPixel(INT8U bprthr, 
	INT8U bpgthr, 
	INT8U bpbthr)
{
	R_CDSP_BADPIX_CTHR	= bprthr & 0xFF | ((INT32U) bpgthr & 0xFF) << 8 | ((INT32U) bpbthr & 0xFF) << 16;
}
...
optical black set
*********************/
void hwCdsp_SetManuOB(INT8U manuoben, 
	INT16U manuob)
{
...
void hwCdsp_SetAutoOB(INT8U autooben, 
	INT8U obtype, 
	INT8U curfob)
{
...
void hwCdsp_SetOB_HVoffset(INT16U obHOffset, 
	INT16U obVOffset)
{
...
void hwCdsp_SetAverageOB(INT8U wboffseten, 
	INT16U ob_avg_r, 
	INT16U ob_avg_gr, 
	INT16U ob_avg_b, 
	INT16U ob_avg_gb)
...
void hwCdsp_OBRead(INT32U * prvalue, 
	INT32U * pgrvalue, 
	INT32U * pbvalue, 
	INT32U * pgbvalue)
{
...
void hwCdsp_EnableLensCmp(INT8U lcen, 
	INT8U stepfactor)
{
...
void hwCdsp_SetLensCmp(INT16U centx, 
	INT16U centy, 
	INT16U xmoffset, 
	INT8U xminc, 
	INT16U ymoffset, 
...
raw h scaler
***************/
void hwCdsp_EnableRawHScale(INT8U hscale_en, 
...
void hwCdsp_SetRawHScale(INT32U src_hsize, 
	INT32U dst_hsize)
{
	INT32U factor;
...
white balance set
****************************/
void hwCdsp_EnableWbGain(//INT8U wboffseten,
INT8U wbgainen)
{
...
void hwCdsp_SetWbOffset(INT8U wboffseten, 
	INT8U roffset, 
	INT8U groffset, 
	INT8U boffset, 
	INT8U gboffset)
...
void hwCdsp_GetWbOffset(INT8U * pwboffseten, 
	INT8S * proffset, 
	INT8S * pgroffset, 
	INT8S * pboffset, 
	INT8S * pgboffset)
...
void hwCdsp_SetWbGain(INT16U r_gain, 
	INT16U gr_gain, 
	INT16U b_gain, 
	INT16U gb_gain)
{
...
void hwCdsp_GetWbGain(INT8U * pwbgainen, 
	INT16U * prgain, 
	INT16U * pgrgain, 
	INT16U * pbgain, 
	INT16U * pgbgain)
...
void hwCdsp_SetWb_R_B_Gain(INT16U r_gain, 
	INT16U b_gain)
{
	R_CDSP_WHBAL_RSETTING &= ~0x1FF;
	R_CDSP_WHBAL_RSETTING |= r_gain & 0x1FF;
...
gain2
*****************/
void hwCdsp_EnableWbGain2(INT8U wbgain2en)
{
...
void hwCdsp_WbGain2Set(INT16U rgain2, 
	INT16U ggain2, 
	INT16U bgain2)
{
	//vaild when input format is raw.
...
void hwCdsp_WbGain2Read(INT16U * prgain, 
	INT16U * pggain, 
	INT16U * pbgain)
{
...
interpolation set
***************/
void hwCdsp_EnableIntplMir(INT8U intplmirhlen, 
	INT8U intplmirvsel, //suggest set 1
INT8U intplcnt2sel)
{
	R_CDSP_INP_MIRROR_CTRL &= ~0x9F;
	R_CDSP_INP_MIRROR_CTRL |= (intplcnt2sel & 0x1) << 7 | (intplmirvsel & 0x1) << 4 | intplmirhlen & 0x0F;
...
void hwCdsp_IntplThrSet(INT8U int_low_thr, 
	INT8U int_hi_thr)
{
...
void hwCdsp_SetEdgeFilter(INT8U lf00, 
	INT8U lf01, 
	INT8U lf02, 
	INT8U lf10, 
	INT8U lf11, 
...
void gpHalCdspSetEdgeFilter(INT8U index, 
	INT8U L0, 
	INT8U L1, 
	INT8U L2)
{
...
void gpHalCdspGetEdgeFilter(INT8U index, 
	INT8U * L0, 
	INT8U * L1, 
	INT8U * L2)
{
...
void hwCdsp_SetEdgeLCoring(INT8U lhdiv, 
	INT8U lhtdiv, 
	INT8U lhcoring, 
	INT8U lhmode)
{
	R_CDSP_LH_DIV_CTRL	=
		 lhdiv & 0x07 | (lhtdiv & 0x07) << 4 | ((INT32U) lhcoring & 0x1FF) << 8 | ((INT32U) lhmode & 0x1) << 16;
}


void gpHalCdspGetEdgeLCoring(INT8U * lhdiv, 
	INT8U * lhtdiv, 
	INT8U * lhcoring, 
	INT8U * lhmode)
{
...
void hwCdsp_SetEdgeAmpga(INT8U ampga, 
	INT8U edgedomain)
{
...
edge lut table set
**********************/
void hwCdsp_InitEdgeLut(INT8U * pLutEdgeTable)
{
	INT32S i;
...
void hwCdsp_EnableEdgeLutTable(INT8U eluten)
{
	if (eluten)
...
color matrix set
***************/
void hwCdsp_EnableColorMatrix(INT8U colcorren)
{
	if (colcorren)
	{
...
void hwCdsp_SetColorMatrix(INT32S a11, 
	INT32S a12, 
	INT32S a13, 
	INT32S a21, 
	INT32S a22, 
...
*******************/
void hwCdsp_SetRawSpecMode(INT8U rawspecmode)
{
	if (rawspecmode > 6)
		return;
...
*****************/
void hwCdsp_RotateModeSet(INT8U rotmode)
{ //not support
...
**********************/
void hwCdsp_EnableYuv444Insert(INT8U yuvinserten)
{
	if (yuvinserten)
...
void hwCdsp_SetYuvCoring(INT8U y_corval_coring, 
	INT8U u_corval_coring, 
	INT8U v_corval_coring)
{
	R_CDSP_YUV_CORING_SETTING =
		 y_corval_coring & 0xFF | (u_corval_coring & 0xFF) << 8 | (v_corval_coring & 0xFF) << 16;
}
...
crop set
*********/
void hwCdsp_SetCrop(INT16U hoffset, 
	INT16U voffset, 
	INT16U hsize, 
	INT16U vsize)
{
...
void hwCdsp_EnableCrop(INT8U hvEnable)
{
	if (hvEnable)
		hvEnable = 0x3;
...
void hwCdsp_CropGet(INT32U * hoffset, 
	INT32U * voffset, 
	INT32U * hsize, 
	INT32U * vsize)
{
...
YUV havg set
***************/
void hwCdsp_SetYuvHAvg(INT8U yuvhavgmiren, 
	INT8U ytype, 
	INT8U utype, 
	INT8U vtype)
{
	R_CDSP_YUV_AVG_LPF_TYPE =
		 ytype & 0x03 | (utype & 0x03) << 2 | (vtype & 0x03) << 4 | ((INT16U) yuvhavgmiren & 0x03) << 8;
...
*******************/
void hwCdsp_SetYuvSpecMode(INT8U yuvspecmode)
{
...
void hwCdsp_SetYuvSpecModeBinThr(INT8U binarthr)
{
...
*****************************/
void hwCdsp_BriContSet(INT32U yb, 
	INT32U yc)
{
...
void hwCdsp_EnableBriCont(INT8U YbYcEn)
{
...
void hwCdsp_SetYuvSPEffOffset(INT8S y_offset, 
	INT8S u_offset, 
	INT8S v_offset)
{
	R_CDSP_YUVSP_EFFECT_OFFSET =
		 ((INT32U) y_offset & 0xFF) | ((INT32U) u_offset & 0xFF) << 8 | ((INT32U) v_offset & 0xFF) << 16;
}


void gpHalCdspGetYuvSPEffOffset(INT8S * y_offset, 
	INT8S * u_offset, 
	INT8S * v_offset)
{
...
void hwCdsp_SetYuvSPEffScale(INT8U y_scale, 
	INT8U u_scale, 
	INT8U v_scale)
{

	R_CDSP_YUVSP_EFFECT_SCALER =
		 ((INT32U) y_scale & 0xFF) | ((INT32U) u_scale & 0xFF) << 8 | ((INT32U) v_scale & 0xFF) << 16;
}


void gpHalCdspGetYuvSPEffScale(INT8U * y_scale, 
	INT8U * u_scale, 
	INT8U * v_scale)
{
...
*****************************/
void hwCdsp_SatHueSet(INT32U uv_sat_scale, 
	INT32U uoffset, 
	INT32U voffset, 
	INT32U u_huesindata, 
	INT32U u_huecosdata, 
...
void hwCdsp_SetYuvSPHue(INT16U u_huesindata, 
	INT16U u_huecosdata, 
	INT16U v_huesindata, 
	INT16U v_huecosdata)
{
...
void gpHalCdspGetYuvSPHue(INT8S * u_huesindata, 
	INT8S * u_huecosdata, 
	INT8S * v_huesindata, 
	INT8S * v_huecosdata)
{
...
yuv h & v scale
***************/
void hwCdsp_EnableYuvHScale(INT8U yuvhscale_en, 
...
void hwCdsp_EnableYuvVScale(INT8U vscale_en, 
...
void hwCdsp_SetYuvHScale(INT16U hscaleaccinit, 
...
void hwCdsp_SetYuvVScale(INT16U vscaleaccinit, 
...
Suppression set
***************/
void hwCdsp_EnableUvSuppr(INT8U suppressen)
{
	if (suppressen)
...
void hwCdsp_SetUvSuppr(INT8U yuvsupmirvsel, 
	INT8U fstextsolen, 
	INT8U yuvsupmiren)
{
	R_CDSP_YUV_RANGE	&= 0x20FF;
	R_CDSP_YUV_RANGE	|=
		 (yuvsupmiren & 0x0F) << 8 | ((INT16U) yuvsupmirvsel & 0x1) << 12 | ((INT16U) fstextsolen & 0x1) << 13;
}
...
* @return	none
*/
void hwCdsp_EnableYEdgeUvSuppr(INT8U posyedgeen)
{
	if (posyedgeen)
...
******************************/
void hwCdsp_EnableYDenoise(INT8U denoisen)
{
	if (denoisen)
...
void hwCdsp_SetYDenoise(INT8U denoisethrl, 
	INT8U denoisethrwth, 
	INT8U yhtdiv)
{
	R_CDSP_DENOISE_SETTING =
		 ((INT16U) yhtdiv & 0x07) << 12 | ((INT16U) denoisethrwth & 0x07) << 8 | denoisethrl & 0xFF;
}
...
************************/
void hwCdsp_EnableYLPF(INT8U lowyen)
{
	if (lowyen)
...
auto focuse / af set
****************************/
void hwCdsp_EnableAF(INT8U af_en, 
	INT8U af_win_hold)
{
...
void hwCdsp_SetAfWin1(INT16U hoffset, 
	INT16U voffset, 
	INT16U hsize, 
	INT16U vsize)
{
...
void hwCdsp_SetAfWin2(INT16U hoffset, 
	INT16U voffset, 
	INT16U hsize, 
	INT16U vsize)
{
...
void hwCdsp_SetAfWin3(INT16U hoffset, 
	INT16U voffset, 
	INT16U hsize, 
	INT16U vsize)
{
...
****************************/
void hwCdsp_EnableAWB(INT8U awb_win_en, 
	INT8U awb_win_hold)
{
...
void hwCdsp_SetAWB(INT8U awbclamp_en, 
	INT8U sindata, 
	INT8U cosdata, 
	INT8U awbwinthr)
{
	R_CDSP_AWB_WIN_CTRL =
		 (sindata & 0xFF) | ((INT32U) cosdata & 0xFF) << 8 | ((INT32U) awbwinthr & 0xFF) << 16 | ((INT32U) awbclamp_en & 0x1) << 24;
}


void hwCdsp_SetAWBThr(INT32U Ythr, 
	INT32U UVL1N1, 
	INT32U UVL1N2, 
	INT32U UVL1N3)
{
	...
	INT16S VL1P1)
{
	...
	//VL1N1 >>= 1;
	//VL1P1 >>= 1;	
	R_CDSP_AWB_SPECWIN_UV_THR1 =
		 (UL1N1 & 0xFF) | ((INT32S) UL1P1 & 0xFF) << 8 | ((INT32S) VL1N1 & 0xFF) << 16 | ((INT32S) VL1P1 & 0xFF) << 24;
	...
	INT16S VL1P2)
{
	...
	//VL1N2 >>= 1;
	//VL1P2 >>= 1;
	R_CDSP_AWB_SPECWIN_UV_THR2 =
		 (UL1N2 & 0xFF) | ((INT32S) UL1P2 & 0xFF) << 8 | ((INT32S) VL1N2 & 0xFF) << 16 | ((INT32S) VL1P2 & 0xFF) << 24;
}
	...
	INT16S VL1P3)
{
	...
	//VL1N3 >>= 1;
	//VL1P3 >>= 1;
	R_CDSP_AWB_SPECWIN_UV_THR3 =
		 (UL1N3 & 0xFF) | ((INT32S) UL1P3 & 0xFF) << 8 | ((INT32S) VL1N3 & 0xFF) << 16 | ((INT32S) VL1P3 & 0xFF) << 24;
}
...
auto expore / ae set
****************************/
void hwCdsp_SetAEAWB(INT8U raw_en, //1:raw(from awblinectr), 0:rgb(poswb) ae/awb window
...
)
{
	...
	R_CDSP_AE_AWB_WIN_CTRL |= ((INT32U) raw_en & 0x1) << 12 | ((INT32U) subample & 0x3) << 13;
}
...
void hwCdspGet3ATestWinEn(INT8U * AeWinTest, 
	INT8U * AfWinTest)
{
...
void hwCdsp_EnableAE(INT8U ae_win_en, 
	INT8U ae_win_hold)
{
	R_CDSP_AE_AWB_WIN_CTRL &= ~0x0111;
	R_CDSP_AE_AWB_WIN_CTRL |= ae_win_hold & 0x1 | ((INT16U) ae_win_en & 0x1) << 8;
...
void hwCdsp_SetAEWin(INT8U phaccfactor, 
	INT8U pvaccfactor)
{
	...
	if (phaccfactor <= 4)
		h_factor = 0;
	else if (phaccfactor <= 8)
		h_factor = 1;
	else if (phaccfactor <= 16)
		h_factor = 2;
	else if (phaccfactor <= 32)
		h_factor = 3;
	else if (phaccfactor <= 64)
		h_factor = 4;
	else if (phaccfactor <= 128)
		h_factor = 5;
	else 
		h_factor = 5;
	...
	if (pvaccfactor <= 4)
		v_factor = 0;
	else if (pvaccfactor <= 8)
		v_factor = 1;
	else if (pvaccfactor <= 16)
		v_factor = 2;
	else if (pvaccfactor <= 32)
		v_factor = 3;
	else if (pvaccfactor <= 64)
		v_factor = 4;
	else if (pvaccfactor <= 128)
		v_factor = 5;
	else 
		v_factor = 5;
...
void hwCdsp_SetAEBuffAddr(INT32U winaddra, 
	INT32U winaddrb)
{
...
void hwCdsp_SetRGBWin(INT16U hwdoffset, 
	INT16U vwdoffset, 
	INT16U hwdsize, 
	INT16U vwdsize)
{
...
void hwCdspGetRGBWin(INT16U * hwdoffset, 
	INT16U * vwdoffset, 
	INT16U * hwdsize, 
	INT16U * vwdsize)
{
...
void hwCdsp_Enable3ATestWin(INT8U AeWinTest, 
	INT8U AfWinTest)
{
...
histgm set
****************************/
void hwCdsp_EnableHistgm(INT8U his_en, 
	INT8U his_hold_en)
{
...
void hwCdsp_SetHistgm(INT8U hislowthr, 
	INT8U hishighthr)
{
	R_CDSP_HISTGM_CTRL	&= ~0xFFFF;
	R_CDSP_HISTGM_CTRL	|= hislowthr | (INT16U)
	hishighthr << 8;
}


void hwCdsp_GetHistgm(INT32U * hislowcnt, 
	INT32U * hishighcnt)
{
...
*/
INT32S gpHalCdspGetAwbSumCnt(INT8U section, 
	INT32U * sumcnt)
{
	//volatile INT32U *ptr;
	if (section == 1)
...
* @return	SUCCESS/ERROR
*/
INT32S gpHalCdspGetAwbSumG(INT8U section, 
	INT32U * sumgl, 
	INT32U * sumgh)
{
	if (section == 1)
...
* @return	SUCCESS/ERROR
*/
INT32S gpHalCdspGetAwbSumRG(INT8U section, 
	INT32U * sumrgl, 
	INT32S * sumrgh)
{
	//volatile INT32U *ptr;
...
* @return	SUCCESS/ERROR
*/
INT32S gpHalCdspGetAwbSumBG(INT8U section, 
	INT32U * sumbgl, 
	INT32S * sumbgh)
{
	//volatile INT32U *ptr;
...
{
	R_ISP_IMSIZE_CROSSTALK_WEIGHT = hsize & 0xFFF | (INT32U)
	vsize << 16;
}
...
****************************/
void hwCdsp_LinCorr_ReadTable(INT8U * rgb_tbl, 
	INT32U len)
{
	int i;
	INT32U reg_tmp;
...
{
	if (dpc_en == 1)
	{ //Open bad pixel algorithm should be enable matsumoto algorithm
...
{
	R_ISP_HR_CROSSTALK_THOLD =
		 (ctk_thr1 & 0xFFFF) | ((INT32U) ctk_thr2 & 0xFFFF) << 8 | ((INT32U) ctk_thr3 & 0xFFFF) << 16 | ((INT32U) ctk_thr4 & 0xFFFF) << 24;
}
...
void hwIsp_CrostlkWeight(INT16U ctkw1, INT16U ctkw2, INT16U ctkw3)
{
	...
	R_ISP_HRR_DENOISE_CROSSTALK_WEIGHT |=
		 ((INT32U) ctkw1 & 0xF) << 16 | ((INT32U) ctkw2 & 0xF) << 20 | ((INT32U) ctkw3 & 0xF) << 24;
}
...
{
	R_ISP_HR_DENOISE_THOLD =
		 (rdn_thr1 & 0xFFFF) | ((INT32U) rdn_thr2 & 0xFFFF) << 8 | ((INT32U) rdn_thr3 & 0xFFFF) << 16 | ((INT32U) rdn_thr4 & 0xFFFF) << 24;
}
...
void hwIsp_DenoiseWeight(INT16U rdnw1, INT16U rdnw2, INT16U rdnw3)
{
	...
	R_ISP_HRR_DENOISE_CROSSTALK_WEIGHT |=
		 ((INT32U) rdnw1 & 0xF) | ((INT32U) rdnw2 & 0xF) << 4 | ((INT32U) rdnw3 & 0xF) << 8;
}
...
**********************/
void hwCdsp_NewDEdgeLut(INT8U * pLutNewDEdgeTable)
{
	INT32S i;
...
* @return	none
*/
void hwCdsp_SetNewDenoise_Sel_Mirror(INT8U ndmirvsel, 
	INT8U ndmiren)
{
	ndmirvsel			&= 0x1;
	ndmiren 			&= 0x0F;
...
* @return	none
*/
void gpHalCdspGetNewDenoise(INT8U * ndmirvsel, 
	INT8U * ndmiren)
{
...
* @return	none
*/
void hwCdsp_EnableNdEdge(INT8U ndedgeen, 
	INT8U ndeluten)
{
	if (ndedgeen)
...
* @return	none
*/
void gpHalCdspGetNdEdgeEn(INT8U * ndedgeen, 
	INT8U * ndeluten)
{
...
* @return	none
*/
void hwCdsp_SetNdEdgeFilter(INT8U index, 
	INT8U L0_SignBit, 
	INT8U L0, 
	INT8U L1_SignBit, 
	INT8U L1, 
...
* @return	none
*/
void gpHalCdspSetNdEdgeFilter(INT8U index, 
	INT8U L0, 
	INT8U L1, 
	INT8U L2)
{
...
void gpHalCdspGetNdEdgeFilter(INT8U index, 
	INT8U * L0, 
	INT8U * L1, 
	INT8U * L2)
{
...
* @return	none
*/
void hwCdsp_SetNdEdgeLCoring(INT8U ndlhdiv, 
	INT8U ndlhtdiv, 
	INT8U ndlhcoring)
{
	/*
...
* @return	none
*/
void gpHalCdspGetNdEdgeLCoring(INT8U * ndlhdiv, 
	INT8U * ndlhtdiv, 
	INT8U * ndlhcoring)
{
