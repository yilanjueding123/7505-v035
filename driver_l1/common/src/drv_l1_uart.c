...
static OS_EVENT * sw_uart_sem[UART_MAX] =
{
	NULL
};
...
INT32S uart_sw_fifo_get(INT8U uart_num, INT8U * data)
{
	...
	if (sw_fifo_head[uart_num].p_write == sw_fifo_head[uart_num].p_read)
	{ // FIFO is empty
...
void uart_data_send(INT8U uart_num, INT8U data, INT8U wait)
{
	...
	if (wait)
	{
		while (pUart->STATUS & C_UART_STATUS_TX_FULL)
