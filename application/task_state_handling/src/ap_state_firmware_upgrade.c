...
void ap_state_firmware_upgrade(void)
{
	...
	while (complete_size < total_size)
	{
		...
		// DBG_PRINT("S:%d\r\n",block_size);
		buffer_left 		=
			 (total_size - complete_size + (C_UPGRADE_SPI_BLOCK_SIZE - 1)) &~(C_UPGRADE_SPI_BLOCK_SIZE - 1);
		...
		while (buffer_left && retry < C_UPGRADE_FAIL_RETRY)
		{
			...
			{ // verify stage
				...
				for (j = C_UPGRADE_SPI_BLOCK_SIZE; j; j -= C_UPGRADE_SPI_WRITE_SIZE)
				{
					...
					for (i = 0; i < 5; ++i)
					{
						if ((SPIFC_Flash_read_page(SPIFC_RESOURCE_OFFSET + complete_size_bak, (INT8U *) verify_buffer)) ==
							 0)
							break;
					}
					...
					if (memcmp((void *) verify_buffer, (void *) (firmware_buffer + ((complete_size_bak & (C_UPGRADE_BUFFER_SIZE - 1)) >> 2)), C_UPGRADE_SPI_WRITE_SIZE) !=
						 0)
					{
					...
					if (flag == 1)
					{
						if (spifc_rewrite(SPIFC_RESOURCE_OFFSET + complete_size_bak, (INT8U *) (firmware_buffer + ((complete_size_bak & (C_UPGRADE_BUFFER_SIZE - 1)) >> 2)), (INT8U *) verify_buffer) !=
							 0)
						{
							break;
						}
	...
	if (retry != C_UPGRADE_FAIL_RETRY)
	{
		...
		//remove((char *)file_info.f_name); //刉壺汔撰恅璃
