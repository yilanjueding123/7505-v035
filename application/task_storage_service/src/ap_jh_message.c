...
/****************************************************************/
static void save_message_to_disk(void)
{
	INT16S fd;
	INT8U* p;
	INT32U addr, len;
	...
	fd = open("C:\\Message.txt", O_RDWR|O_TRUNC|O_CREAT);
	if(fd < 0)
		return;
	
	//copyright mssage
	len = strlen(R_CopyrightMSG_Buf)+1;
	addr = (INT32U)gp_malloc(len);
	if(!addr)
	{
		close(fd);
		return;
	...
	p = (INT8U*)addr;
	sprintf((char *)p, (const char *)"%08d,JH%04d,v00,", data_num, product_num);
	p[17] = ((PROGRAM_VERSION_NUM%1000)/100) + '0';
	p[19] = ((PROGRAM_VERSION_NUM%100)/10) + '0';
	p[20] = ((PROGRAM_VERSION_NUM%10)/1) + '0';
	write(fd, addr, 22);
...
Fail_Return_2:
	...
	read_flag			= 1;
	/*****************************************************/
	nRet 				= gp_strncmp((INT8S *) addr, (INT8S *) "Message?", 7);
	if(nRet == 0)
	{
		save_message_to_disk();
		gp_free((void *) addr);
