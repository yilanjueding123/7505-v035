...
static INT8U  switch_key_prev_state=0;
#if TV_DET_ENABLE
...
void ap_peripheral_key_init(void)
{
	...
	switch_key_prev_state=switch_key_state;
}
...
void ap_peripheral_key_judge(void)
{
	...
	if (++switch_key_read_cnt > 16)
	{
		...
		if (switch_key_state != switch_key_prev_state)
		{
			switch_key_prev_state = switch_key_state;

			if (switch_key_state) //拨到录像
			{
				msgQSend(ApQ, MSG_APQ_VIDEO_RECORD_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
			}
			else //拨到拍照
				...
				if (ap_state_handling_storage_id_get() != NO_STORAGE)
				{
					if (switch_state_get() == 1)
					{
						...
						if (*tick_cnt_ptr == 24)
						{
							...
							{
								if (((video_record_sts & 0x02) == 0) && (video_record_sts & 0x04))
								{
								...
								else if (((video_record_sts & 0x02)) && (video_record_sts & 0x04))
								{
									msgQSend(ApQ, MSG_APQ_VIDEO_RECORD_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
								}
							}
